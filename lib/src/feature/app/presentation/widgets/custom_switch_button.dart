          ...
          AnimatedOpacity(
            ...
            child: AnimatedDefaultTextStyle(
              style: DefaultTextStyle.of(context).style.merge(
                    TextStyle(
                      fontWeight: widget.highlighted ? FontWeight.w500 : FontWeight.normal,
                    ),
                  ),
...
class _SegmentedControlState<T> extends State<CustomSwitchButton<T>>
  ...
  void onUpdate(DragUpdateDetails details) {
    if (isThumbDragging) {
      ...
      final T? segment = _hasDraggedTooFar(details) ? null : segmentForXPosition(details.localPosition.dx);
      onPressedChangedByGesture(segment);
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return UnconstrainedBox(
      ...
      child: Container(
        padding: widget.overallPadding ?? widget.padding.resolve(Directionality.of(context)),
        decoration: BoxDecoration(
          // border: Border.all(
          //   width: 0,
          //   color: context.theme.whenByValue(
          //     light: AppLightColors.day200,
          //     dark: AppDarkColors.night200,
          //   ),
          // ),
    ...
    if (highlightedChildIndex != null) {
      ...
      if (state.thumbController.isAnimating) {
        ...
        if (thumbTween == null) {
          ...
          final Rect startingRect = moveThumbRectInBound(currentThumbRect, children) ?? newThumbRect;
          state.thumbAnimatable = RectTween(begin: startingRect, end: newThumbRect).chain(
